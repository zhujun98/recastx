...
Eigen::RowVector3f Path3::operator()(float param) const {
    ...
    // where p = components, s = param in [0, 1], and a, b as below.
...
std::ostream& operator<<(std::ostream& out, Path3 const& p) {
    out << "Path3, using components" << std::endl
        << p.nodes() << std::endl
        << "and " << p.bdry_conds();
    return out;
}
...
Eigen::VectorXf Path3::system_rhs(bdry_cond bc_left, bdry_cond bc_right,
    ...
    // between the next and previous components
    // as right-hand sides.
