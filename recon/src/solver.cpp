...
Solver::Solver(int rows, 
    ...
    spdlog::info("Slice volume: {}", recon::utils::info(*vol_geom_));
    ...
    spdlog::info("Small preview volume: {}", recon::utils::info(*vol_geom_small_));
...
ParallelBeamSolver::ParallelBeamSolver(int rows, 
    ...
    if (!vec_geometry) {
        ...
        proj_geom_ = recon::utils::proj_to_vec(&proj_geom);

        proj_geom_small_ = recon::utils::proj_to_vec(&proj_geom);

    } else {
        auto par_projs = recon::utils::list_to_par_projections(angles);
...
ConeBeamSolver::ConeBeamSolver(int rows, 
    ...
    if (!vec_geometry) {
        ...
        auto cone_projs = recon::utils::list_to_cone_projections(rows, cols, angles);
        proj_geom_ = std::make_unique<astra::CConeVecProjectionGeometry3D>(
            projections_, rows, cols, cone_projs.data());

        spdlog::info("{}", recon::utils::info(*proj_geom_));
