...
class ReconTest : public testing::Test {
  protected:
    ...
    recon::Reconstructor recon_ {rows_, cols_, threads_};

    void SetUp() override {
        angles_ = recon::utils::defaultAngles(group_size_);
        buildRecon();
    ...
    void buildRecon() {
        ...
        recon_.setSolver(std::make_unique<recon::ParallelBeamSolver>(
            rows_, cols_, angles_, 
...
TEST(TestUtils, TestComputeReciprocal) {
    ...
    recon::utils::computeReciprocal(darks, flats, pixels, reciprocal, dark_avg);
